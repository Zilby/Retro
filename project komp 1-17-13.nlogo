...
to second_change
   if Level = ""
   [if coloring = "color" [cro 1 [set size 0 setxy 35 -110 set label "Level One" set label-color 65]
     ask patches [if pxcor < 64 
    and pxcor > -64 and pycor < -90 and pycor
    > -130 [set pcolor 65]]
  ask patches [if pxcor < 60 
    and pxcor > -60 and pycor < -93 and pycor
    > -127 [set pcolor black]] set coloring "no"]
...
to third_change
  ...
  if platform-time > 0 [set velocity 8 set platform-time platform-time - 1 every .3 [ask creators [create_platforms]]]
   if velocity < -18 [dead]
   if time = 0 
     [cro 1 [set breed ships set shape "spaceship" set size 80 rt 90 setxy -200 -20] set time -9001 ask platforms [die]]
     if time = -9001
